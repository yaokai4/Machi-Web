...
export default async function CreateListingRoute({
  ...
  searchParams: Promise<{ type?: string; city?: string; category?: string; edit?: string }>;
}) {
  const params = await searchParams;
  return (
    <Suspense fallback={<RouteFallback title="正在打开发布页" rows={4} />}>
      <CreateListingPage initialType={params.type} initialCitySlug={params.city} initialCategory={params.category} editListingId={params.edit} />
    </Suspense>
  );
}
