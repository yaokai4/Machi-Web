    ...
    <AppShell requireAuth wide right={null}>
      <main className="mx-auto max-w-3xl px-3 py-4 sm:px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-950">时段预约</h1>
            <p className="text-sm font-semibold text-slate-500">为房源 / 服务设置可预约时段,让对方在线选时间</p>
          </div>
        </div>

        <div className="mt-5 space-y-3">
          {loading ? (
            <>{[0, 1, 2].map((i) => <div key={i} className="h-16 animate-pulse rounded-3xl bg-slate-100" />)}</>
          ) : null}

          {!loading && listings.length > 0 ? (
            <p className="text-sm font-semibold text-slate-500">选择一条发布,进入详情即可添加 / 删除可预约时段。</p>
          ) : null}
          ...
          {listings.map((l) => (
            <Link
              ...
              className="flex items-center gap-3 rounded-3xl border border-slate-200/70 bg-white p-4 transition hover:-translate-y-0.5 hover:border-[rgba(var(--kx-living-accent),0.4)]"
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[15px] font-black text-slate-950">{l.title || "未命名发布"}</p>
                <p className="mt-0.5 text-xs font-semibold text-slate-500">管理可预约时段</p>
              </div>
          ...
          {!loading && listings.length === 0 ? (
            <section className="rounded-3xl border border-slate-200/70 bg-white px-5 py-12 text-center">
              ...
              <p className="mt-3 text-base font-black text-slate-950">暂无可设置时段的发布</p>
              <p className="mt-2 text-sm font-semibold text-slate-500">发布租房或本地服务后,就能在详情页设置可预约时段。</p>
              <Link href="/listings/create" className="mt-4 inline-block rounded-full bg-slate-900 px-4 py-2 text-sm font-bold text-white">去发布</Link>
            </section>
          ) : null}
        </div>
      </main>
