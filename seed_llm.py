...
_TYPE_BRIEF: dict[str, str] = {
    ...
    "pitfall": "亲测避坑：第一人称踩坑警告，先说自己错在哪、亏了多少钱/时间，再给正确做法，带具体金额和环节，让人想收藏转发",
    "checklist": "实用清单：150-280字纯清单帖「XX前必做N件事」，每条一个可执行动作/材料/顺序，一眼可照做，不灌水",
    "experience": "经验分享：80-200字第一人称单点经验，每条必须带一个真信息点（地点/价格/步骤/时间），不是emo灌水",
}
...
SPOTLIGHT_THEMES: tuple[str, ...] = (
    ...
    "日本超市减价时段与标签怎么看", "在家做中餐去哪买食材",
    # 住
    "第一次租房避坑全攻略", "UR 团地值不值得", "share house 真实体验", "租房初期费用清单", "退租押金怎么要回",
    "怎么挑安静不踩雷的房子", "搬家流程与省钱", "水电煤气网络怎么开通", "保证公司/连带保证人怎么搞",
    "家具家电去哪买最划算",
    # 行
    "交通 IC 卡怎么用最划算", "JR Pass 到底值不值", "夜行巴士体验", "市内骑自行车注意事项", "机场到市区怎么走最省",
    "末班车与深夜回家", "定期券怎么买最省", "新干线抢票与早特价", "打车/叫车 App 怎么用",
    # 玩乐
    "东京近郊一日游路线", "温泉怎么泡更舒服", "赏樱地图", "赏枫地图", "美术馆/展览推荐", "祭典与花火大会",
    "迪士尼/环球省钱攻略", "镰仓/箱根/河口湖怎么玩", "滑雪场新手怎么选", "海边/离岛怎么去",
    # 衣 + 网购
    "换季穿搭与买衣服", "二手店/古着淘宝", "药妆护肤必买", "メルカリ 买卖入门", "亚马逊日本/乐天怎么薅",
    "优衣库/GU/无印良品怎么买省", "大件家电网购避坑", "海淘/转运回国怎么弄",
    # 办事·行政
    "第一个月必办清单", "区役所手续怎么办", "在留卡更新/变更流程", "マイナンバー 卡怎么办",
    "住民票/印鉴登录怎么弄", "垃圾分类完全指南", "搬家后地址变更一条龙", "在日证件照哪拍便宜",
    "行政窗口中文/翻译怎么找",
    # 税务·金融
    "确定申告怎么报", "年末调整是什么怎么弄", "住民税怎么算怎么交", "ふるさと納税 值不值", "银行开户怎么选",
    "第一张信用卡怎么办", "往国内汇款怎么最省手续费", "在日理财/NISA 入门",
    # 医疗·健康
    "看病就医流程", "怎么找会中文的医院诊所", "牙科/眼科怎么预约", "国民健康保险与高额疗养费",
    "常备药与药店买药指南", "心理咨询/情绪低落怎么办", "体检/健康诊断怎么约",
    # 手机·网络
    "手机卡运营商对比", "格安 SIM 怎么选", "家里宽带怎么装最省", "手机分期与两年缚约避坑", "回国后号码怎么处理",
    # 考学
    "语言学校怎么选", "大学院出愿流程", "EJU/留考备考", "研究计划书怎么写", "怎么联系教授拿内诺",
    "面接经验分享", "奖学金怎么申请",
    # 就职
    "在日转职流程", "履历书/职务经历书怎么写", "就职面接常见问题", "工作签证更新", "派遣 vs 正社员",
    "双语/IT 岗位怎么找", "内定后入职前要办什么", "转职中介/招聘网怎么用",
    # 育儿·家庭
    "保育园/幼稚园怎么申请", "怀孕生产在日流程与补助", "儿童医疗费与儿童手当", "带娃周末去哪玩",
    "国际结婚/配偶签证手续",
    # 签证·身份
    "各类在留资格区别与转换", "永住申请条件与流程", "归化入籍是怎么回事", "签证到期前要做什么",
    # 生活·防灾·心理
    "驾照怎么考或换发", "宠物在日怎么养与租房", "台风地震防灾准备", "花粉症/季节过敏怎么扛",
    "一个人在日孤独怎么调节", "回国或长期离开前的清单", "怎么交朋友找组织不孤单",
)
...
def _build_prompt(*, region_code: str, language: str, tone: str, plan: dict[str, int],
    ...
    # The batch "angle" (语气). Previously `tone` was passed in but never used —
    # this directive is what makes each angle actually change the output.
    tone_directive = seedlib.TONE_DIRECTIVE.get(tone, "")
    ...
    system = (
        ...
        "9) 信息可信、不编造——具体数字(价格/时间/距离)给「大概/我上次是」而非斩钉截铁；涉及手续/金额/截止日提醒「以官方/役所/合同最新为准」；"
        "绝不编造精确的真实店名+地址+价位当既定事实（可写「车站附近有几家平价的」但别点名「XX店680日元」当权威）。\n"
    )
    # Inject the batch angle (the previously-dead `tone`) right before the
    # output instruction, so it demonstrably steers generation.
    if tone_directive:
        system += tone_directive + "\n"
    system += "只输出 JSON，不要解释。"

    spotlight_note = (
        "\n【spotlight = 精选攻略·干货长帖，这是重点。目标：比小红书高赞干货帖更实用、更可信、更没水分】：\n"
        "- 长度 380-720 字，围绕【一个】清晰主题真正讲透，别面面俱到跑题。\n"
        "- 开头一句真人化引子（自己踩过的坑/被问烦了/花了冤枉钱才搞懂），别用「干货分享」「手把手教你」这种标题党。\n"
        "- 主体用「1️⃣ 2️⃣ 3️⃣」或短换行分 3-6 点。**每一点必须给可执行信息**：具体车站/线路/区域、价格区间(日元)、耗时、步骤顺序、需带材料、判断标准。没有具体信息的点宁可删。\n"
        "- 至少一处「个人取舍」：我一般选…因为…、这步我会跳过因为…、亲测这样最省。要有人味有判断，不是清单堆砌。\n"
        "- 至少一处「时效/可信度锚」：今年/上个月我去的时候、以官方最新为准；涉及金额手续一定提醒去官方确认。\n"
        "- 可有一两句真实吐槽或情绪，但不喧宾夺主。\n"
        "- 结尾自然收（有具体情况评论区问我/先码后看/我踩过的坑就这些），不要总结升华、不要「希望对你有帮助」客套。\n"
        "- 绝不：编造精确的真实店名+地址+价位当权威；罗列「正确的废话」没信息量；写成官方说明书或 SEO 文。\n"
        + (
            "- 这一批每条分别写下面这些【互不相同】的主题（一条对应一个，绝不重复、绝不都写「周末怎么安排」）："
            + "；".join(themes) + "。\n"
            if themes else
            "- 每条主题必须完全不同（衣/食/住/行/玩乐/考学/就职/医疗/签证/税务/育儿都可以），绝不重复套同一个模板。\n"
        )
        + "- 自检：写完想一下「一个刚来日本的人读完，能立刻多做对一件事吗？」不能就重写。\n"
