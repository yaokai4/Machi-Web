...
USERS = [
    ...
    ("foodwalk", "约饭探店", "周末约饭、咖啡、探店和城市散步。", "fork.knife", "pink", "free", 0),
    ...
    ("weekend_buddy", "周末搭子", "饭搭子、摄影搭子、运动搭子和语言交换。", "person.2.fill", "cyan", "free", 0),
...
def channel_payloads(city_name: str, topic: str) -> list[dict[str, Any]]:
    return [
        ...
        {
            "type": "meetup",
            "author": "weekend_buddy",
            "title": f"{city_name}周末城市散步搭子",
            "content": f"这个周末想在{city_name}约一个城市散步搭子，从咖啡店开始，沿着生活气息比较强的街区慢慢走，顺手拍点店招、路口、市场和居民区的小细节。节奏不赶，适合想熟悉城市、练习拍照、顺便聊天的人。出发前会共享路线和集合点，安全第一，迟到提前说。#{topic} #搭子 #Citywalk",
            "attrs": {"title": "周末城市散步搭子", "meetup_type": "Citywalk / 摄影", "meetup_time": "周六下午", "location": city_name, "people_limit": 4, "budget": "咖啡自理", "description": "慢走、拍照、聊天，安全集合。", "safety_notice": "公共场所集合，路线提前共享。"},
        },
        {
            "type": "dining",
            "author": "foodwalk",
            "title": f"{city_name}约饭：想试一家口碑小店",
            "content": f"想在{city_name}约两三个人一起吃饭，目标是口碑稳定但不用排太久的小店。预算控制在普通工作日晚餐水平，大家可以提前说忌口、预算和是否能喝酒。希望是轻松聊天局，不推销、不迟到、不临时消失。#{topic} #约饭 #探店",
            "attrs": {"title": "约饭：口碑小店", "restaurant_or_area": city_name, "meetup_time": "工作日晚上或周末中午", "people_limit": 4, "budget": "人均可商量", "description": "轻松聊天，提前确认忌口和预算。"},
        },
        {
            "type": "event",
            "author": "eventline",
        ...
        },
        {
            ...
            "content": f"提供{city_name}本地搬家、地址变更、手机卡、网络、简单翻译和资料整理协助。服务会先确认需求清单、时间、预算和是否需要现场陪同，再给出可执行方案。不承诺不合规事项，费用透明，复杂问题建议咨询专业人士。#{topic} #本地服务 #搬家",
