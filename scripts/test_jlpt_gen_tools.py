...
class AssembleBankTests(unittest.TestCase):
    ...
    def test_seed_accepts_three_section_n3_manifest_and_only_first_section_charges(self) -> None:
        ...
        parent, vocab_child, grammar_reading, listening = payloads
        self.assertEqual("paper", parent["kind"])
        self.assertEqual(
            ["vocab", "", "listening"],
            [vocab_child["section"], grammar_reading["section"], listening["section"]],
        )
        self.assertEqual([250, 0, 0], [
            vocab_child["coinCost"], grammar_reading["coinCost"], listening["coinCost"]
        ])
        self.assertEqual(["percent", "percent", "percent"], [
            vocab_child["scoreMode"], grammar_reading["scoreMode"], listening["scoreMode"]
        ])
